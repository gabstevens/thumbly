...
describe("ThumblyClient", () => {
  ...
  it("should throw error if neither driver nor supabase config is provided", () => {
    ...
    expect(() => new ThumblyClient({ surveyId: "test" })).toThrow("ThumblyClient: Invalid configuration");
  });
  ...
  it("should retry on TransientError", async () => {
    ...
    mockDriver.submitVote = vi.fn().mockRejectedValueOnce(error).mockResolvedValueOnce(undefined);
